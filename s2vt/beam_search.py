...
class EncoderRNN(nn.Module):
    ...
    def forward(self, input):
        #input_lengths = input_lengths.cpu().int()
        # Embed input
    ...
    #     # Embed input
    #     embedded = self.dropout(self.embedding(input))
    #     # Pack the padded sequence of frames (required for variable-length sequences)
    #     packed = pack_padded_sequence(embedded, input_lengths, batch_first=True, enforce_sorted=False)
    #     # Process through GRU
    #     packed_output, hidden = self.gru(packed)

    #     # Unpack sequence back into padded form
...
class DecoderAttention(nn.Module):
    ...
    def forward_step(self, input, hidden, encoder_outputs):
        ...
        # print("embedded size dec: ",embedded.size())
        # print("context size dec: ",context.size())
...
class VideoAnalysisModel(nn.Module):
    ...
    def forward(self, video_frames, target_tensor=None, caption_length=30):
        ...
        encoder_output, encoder_hidden = self.encoder(cnn_features)  #bener pake caption length?
        decoder_output, decoder_hidden, attn_weights = self.decoder(encoder_output, encoder_hidden, target_tensor)

        return decoder_output, attn_weights

def beam_search_decoder(model, video_frames, vocab, beam_width=3, max_len=30):
    # Move video frames to device (GPU/CPU)
    video_frames = video_frames.to(device)
    
    # Reshape video_frames from [batch_size, num_frames, channels, height, width]
    batch_size, num_frames, channels, height, width = video_frames.size()
    video_frames = video_frames.view(batch_size * num_frames, channels, height, width)
    
    # Extract CNN features
    with torch.no_grad():
        cnn_features = model.cnn(video_frames)
    
    # Reshape CNN features back to [batch_size, num_frames, feature_size]
    cnn_features = cnn_features.view(batch_size, num_frames, -1)

    # Pass through EncoderRNN
    encoder_outputs, encoder_hidden = model.encoder(cnn_features)

    # Initialize the beam with the start token
    start_token = vocab.word2idx["<start>"]
    end_token = vocab.word2idx["<end>"]
    
    beams = [(0, [start_token], encoder_hidden)]  # (score, sentence, hidden state)

    # Beam search
    for _ in range(max_len):
        all_candidates = []
        for score, sentence, hidden in beams:
            # Stop if the sentence is completed
            if sentence[-1] == end_token:
                all_candidates.append((score, sentence, hidden))
                continue
            
            # Get the last word in the sentence and run the decoder
            last_word = torch.tensor([[sentence[-1]]], device=device)
            decoder_output, hidden, _ = model.decoder(encoder_outputs, hidden, last_word)
            log_probs = F.log_softmax(decoder_output, dim=-1)

            # Expand each beam with the top `beam_width` words
            top_log_probs, top_words = log_probs.topk(beam_width, dim=-1)

            for i in range(beam_width):
                candidate_score = score + top_log_probs[0, 0, i].item()
                candidate_sentence = sentence + [top_words[0, 0, i].item()]
                all_candidates.append((candidate_score, candidate_sentence, hidden))

        # Sort all candidates by score and keep only the top `beam_width` beams
        beams = sorted(all_candidates, key=lambda x: x[0], reverse=True)[:beam_width]

    # Return the best sentence (highest score)
    best_score, best_sentence, _ = beams[0]
    decoded_words = [vocab.idx2word[idx] for idx in best_sentence if idx != start_token and idx != end_token]
...
def train_model(model, dataloader, criterion, optimizer, num_epochs, clip = 5,checkpoint_path=None):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        checkpoint_path = f'/home/arifadh/Desktop/checkpoint/beamsearch_pack_padded/checkpoint_epoch_{epoch+1}.pth'
...
def evaluate_with_beam_search(model, video_frames, vocab, beam_width=3, max_len=30):
    #model, video_frames, input_lengths, vocab, beam_width=3, max_len=30
    decoded_sentence = beam_search_decoder(model, video_frames,vocab, beam_width, max_len)
...
checkpoint_path = '/home/arifadh/Desktop/checkpoint/beamsearch_pack_padded/checkpoint_epoch_1.pth'
...
model = VideoAnalysisModel(cnn_model_name="resnet50", cnn_output_size=512, hidden_size=256, output_size=vocab_size).to(device)
...
cnt = 0
for video_frames, caption in evaldata:
    ...
    decoded_sentence, attn_weights = evaluate_with_beam_search(model, video_frames, vocab)
...
##benerin beam search, nanti malem
##Increase GRU layer
##Train CNN from begining,