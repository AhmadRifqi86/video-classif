...
class Cell(nn.Module):
    def __init__(self, in_size, out_size, mode):
        ...
        for char in mode:
            if char == 'l':
                layer_list.append(nn.Linear(self.input_size, self.output_size))
            elif char == 'n':
                layer_list.append(nn.LayerNorm(self.output_size))
            elif char == 'g':
        ...
        self.cell = nn.Sequential(*layer_list)
...
class Adapt(nn.Module):
    def __init__(self, in_size, out_size, mode, depth=3, drop=0.25, factor=2):
        super().__init__()
        self.input_size = in_size
        self.output_size = out_size
        self.mode = mode
        ...
        # Generate size_list dynamically
        size_list = [in_size]
        for i in range(1, depth):
            size_list.append(size_list[-1] // factor)
        size_list.append(out_size)

        # Create the layers using size_list
        layer_list = []
        for i in range(len(size_list) - 1):
            layer_list.append(Cell(size_list[i], size_list[i + 1], mode))
            if drop > 0:
...
#     # Input tensor
#     x = torch.randn(8, 512)  # Batch size 8, input size 512

#     # Instantiate Adapt
#     adapt_layer = Adapt(in_size=512, out_size=128, mode="lgn", depth=3, drop=0.25, factor=2)
#     print(adapt_layer)

#     # Forward pass
#     output = adapt_layer(x)
