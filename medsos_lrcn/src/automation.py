...
#             best_f1 = f1
#             best_acc, best_precision, best_recall = accuracy, precision, recall
#             best_train, best_inf = train_dur, inf_dur
#             best_model_filename = f"best_model_seq{all_config.SEQUENCE_LENGTH}_batch{all_config.CONF_BATCH_SIZE}_hidden{all_config.CONF_HIDDEN_SIZE}_cnn{all_config.CONF_CNN_BACKBONE}_rnn{all_config.CONF_RNN_INPUT_SIZE}_layer{all_config.CONF_RNN_LAYER}_rnnType{all_config.CONF_RNN_TYPE}_f1{f1:.4f}.pth"
#             best_model_path = os.path.join(all_config.BEST_MODEL_DIR, best_model_filename)

#         # Save model if accuracy > 0.76
#         if accuracy > 0.76:
#             model_filename = f"model_seq{all_config.SEQUENCE_LENGTH}_batch{all_config.CONF_BATCH_SIZE}_hidden{all_config.CONF_HIDDEN_SIZE}_cnn{all_config.CONF_CNN_BACKBONE}_rnn{all_config.CONF_RNN_INPUT_SIZE}_layer{all_config.CONF_RNN_LAYER}_rnnType{all_config.CONF_RNN_TYPE}_acc{accuracy:.4f}.pth"
#             model_path = os.path.join(all_config.BEST_MODEL_DIR, model_filename)
#             subprocess.run(f"cp {all_config.MODEL_PATH} {model_path}", shell=True)
...
def run_training(config, test_runs, best_results):
    ...
    for run in range(test_runs):
        ...
        if accuracy > 0.77:
            ...
            print(f"Saving model with accuracy > 0.77 for configuration: {best_model_filename}")
            subprocess.run(f"cp {all_config.MODEL_PATH} {best_model_path}", shell=True)
